...
import ru.kulikov.first_bot.commands.cancel_delete.CancelDelete;
import ru.kulikov.first_bot.commands.delete_category.DeleteCategory;
import ru.kulikov.first_bot.commands.help.HelpCommand;
import ru.kulikov.first_bot.commands.list_expenses.ListExpenses;
import ru.kulikov.first_bot.commands.start.StartCommand;
...
public class Bot extends TelegramLongPollingBot {
    ...
    private static final String[] not_command = new String[2];
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasCallbackQuery()) {
            makeDeleteButton(update);
        }
        else if (update.hasMessage()) {
            user_id = update.getMessage().getChatId();
            Message message = update.getMessage();
            if (!message.hasEntities()) {
                runNotCommand(update, message);
            }
            else {
                logger.info("Execution command: " + update.getMessage().getText());
                String command = message.getText();
                switch (command) {
                    case "/delete_category" -> deleteCategory(message);
                    case "/list_expenses" -> showListExpenses(message);
                    case "/start" -> start(message);
                    case "/help" -> help(message);
                    case "/cancel_delete" -> canselDelete(message);
                }
            }
        }
    }
    ...
    public void makeDeleteButton(Update update) {
    ...
    public void runNotCommand(Update update, Message message) {
    ...
    public void showListExpenses(Message message) {
        try {
            deleteMessageAboutDelete();
    ...
    public void canselDelete(Message message) {
        try {
